...
class JDBCTreeSpec extends Specification{

  "makeClass on a table with a primary key" should{
    val tree = JDBCTree.makeClass("Foo", Column("a", IntType, None, None, PrimaryKey) :: Nil, Nil)
  ...
  "makeClass on a table with more than 22 fields and no primary key" should{
    ...
    val tree = JDBCTree.makeClass("Foo", columns.toList, Nil)
  ...
  "JDBCTree on a table with multiple columns" should{
    ...
    val columns = for{indx <- 1 to 23} yield Column("a" + indx.toString, IntType, None, None, ColumnDef)
    val tree = JDBCTree(Table("Foo", None, columns.toList), "", formato)
  ...
  "JDBCTree on a table with one column" should{

    val tree = JDBCTree(Table("Foo", None, Column("a", IntType, None, None, PrimaryKey) :: Nil), "", formato)
