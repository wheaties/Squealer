...
class ObjectTreeSpec extends Specification{
  "ObjectTree extract" should{
    ...
    "handle non-null fields" in{
      val tree = ObjectTree.extract(Column("a", "Int", None, None, ColumnDef) :: Nil)

      treeToString(tree.head) must be_==("result.getInt(a)")
    }

    "handle null fields" in{
      val tree = ObjectTree.extract(Column("a", "Int", None, None, NullableColumn) :: Nil)

      treeToString(tree.head) must be_==("Option(result.getInt(a))")
    }

    "handle multiple fields" in{
      val tree = ObjectTree.extract(Column("a", "Int", None, None, ColumnDef) :: Column("b", "Int", None, None, ColumnDef) :: Nil)
  ...
  "ObjectTree" should{
    ...
    "handle non-null fields" in{
      val tree = ObjectTree("Foo", Column("a", "Int", None, None, ColumnDef) :: Nil)

      treeToString(tree) must be_==("object Foo {\n  def apply(result: ResultSet) = new Foo(result.getInt(a))\n}")
    }

    "handle null fields" in{
      val tree = ObjectTree("Foo", Column("a", "Int", None, None, NullableColumn) :: Nil)

      treeToString(tree) must be_==("object Foo {\n  def apply(result: ResultSet) = new Foo(Option(result.getInt(a)))\n}")
    }

    "handle multiple fields" in{
      val tree = ObjectTree("Foo", Column("a", "Int", None, None, ColumnDef) :: Column("b", "Int", None, None, ColumnDef) :: Nil)
